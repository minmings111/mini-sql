...
- 지원 테이블: `users` 1개
- 지원 명령: `INSERT`, `SELECT`
- 실행 파일: [sql_processor](/home/leeminjeong/workspace/c_project/my_study/sql_processor)
- 데이터 파일: [users.csv](/home/leeminjeong/workspace/c_project/my_study/data/users.csv)

## 발표용 요약
이 프로젝트는 사용자가 CLI에서 MiniSQL 문장을 입력하면, 프로그램이 그 문장을 파싱해 명령 구조체로 바꾸고, 실행기에서 해당 명령을 처리한 뒤, CSV 파일에 저장하거나 파일에서 조회한 결과를 콘솔에 출력하는 구조로 동작한다.

즉, 작은 규모이지만 DB 처리기의 핵심 단계인 `Read -> Parse -> Execute -> Store -> Print`를 모두 직접 구현한 프로젝트라고 설명할 수 있다.

## 실행 흐름
![MiniSQL execution flow](/home/leeminjeong/workspace/c_project/my_study/assets/minisql-flow.svg)
...
5. `storage`가 [users.csv](/home/leeminjeong/workspace/c_project/my_study/data/users.csv)를 읽거나 쓴다.
6. `printer`가 결과 또는 오류 메시지를 출력한다.

## 함수 단위 세부 흐름
![MiniSQL internal call flow](/home/leeminjeong/workspace/c_project/my_study/assets/minisql-call-flow.svg)

조금 더 코드 가까이에서 보면 호출 관계는 이렇게 이어진다.

1. [main.c](/home/leeminjeong/workspace/c_project/my_study/src/main.c) 의 `main()`이 [repl.c](/home/leeminjeong/workspace/c_project/my_study/src/repl.c) 의 `run_repl()`를 호출한다.
2. `run_repl()`는 `fgets()`로 입력을 받고 `trim_newline()`, `trim_spaces()`, `is_blank_string()`으로 입력을 정리한다.
3. 여러 줄 입력일 수 있으므로 `append_input_line()`으로 버퍼에 누적하고, `has_complete_statement()`로 세미콜론이 나왔는지 확인한다.
4. 문장이 완성되면 `process_input_line()`이 [parser.c](/home/leeminjeong/workspace/c_project/my_study/src/parser.c) 의 `parse_command()`를 호출한다.
...
6. 파싱이 성공하면 `process_input_line()`이 [executor.c](/home/leeminjeong/workspace/c_project/my_study/src/executor.c) 의 `execute_command()`를 호출한다.
7. `execute_command()`는 명령 종류에 따라 `execute_insert()` 또는 `execute_select()`로 분기한다.
8. `execute_insert()`는 `validate_insert_command()`로 값 개수와 숫자 필드를 검사한 뒤, [storage.c](/home/leeminjeong/workspace/c_project/my_study/src/storage.c) 의 `append_user_record()`를 호출한다.
9. `execute_select()`는 `validate_select_command()` 후 `read_all_users()`로 CSV를 읽고, `split_csv_row()`와 `row_matches_condition()`으로 각 행을 검사한다.
10. 조회 결과는 [printer.c](/home/leeminjeong/workspace/c_project/my_study/src/printer.c) 의 `print_select_header()`, `print_user_row()`, `print_rows_selected()`로 출력되고, INSERT 성공 시에는 `print_message()`가 호출된다.
...
- [study_docs](/home/leeminjeong/workspace/c_project/my_study/study_docs/README.md): 초보자용 학습 문서
- [include](/home/leeminjeong/workspace/c_project/my_study/include/constants.h): 공용 헤더 파일
- [src](/home/leeminjeong/workspace/c_project/my_study/src/main.c): 실제 C 소스 코드
- [data](/home/leeminjeong/workspace/c_project/my_study/data/users.csv): CSV 데이터 파일
- [assets](/home/leeminjeong/workspace/c_project/my_study/assets/minisql-flow.svg): README용 다이어그램
