...
5. `storage`가 [users.csv](/home/leeminjeong/workspace/c_project/my_study/data/users.csv)를 읽거나 쓰고, `id` 조회용 B-tree 인덱스를 유지한다.
...
8. `execute_insert()`는 `validate_insert_command()`로 값 개수, 숫자 필드, 중복 `id`를 검사한 뒤, [storage.c](/home/leeminjeong/workspace/c_project/my_study/src/storage.c) 의 `append_user_record()`를 호출한다.
...
- [data/users.idx](/home/leeminjeong/workspace/c_project/my_study/data/users.idx): 인덱스 설명 파일
